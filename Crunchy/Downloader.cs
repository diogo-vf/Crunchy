...
using Crunchy.Configs;
using Crunchy.FFtools;
...
public class Downloader
{
    ...
    public async Task Download(IndexResponse? indexResponse, IEnumerable<string> urls, string locale,
        ProcessPriorityClass? ffmpegPriority)
    {
        List<string> enumerable = urls.ToList();
        ...
        ProcessPriorityClass? ffmpegPriority)
    {
        foreach (string url in urls)
        {
            ...
            foreach (Episode episode in episodes) await DownloadEpisode(episode, ffmpegPriority);
        }
    }
    ...
    private string? SearchInM3U8TheBestDownloadUrl(string? m3U8Url)
    {
        ...
        for (int i = 0; i < matches.Count; i++)
        {
            string strResolution =
                matches[i].Value[(matches[i].Value.LastIndexOf("x", StringComparison.Ordinal) + 1)..];
    ...
    private async Task<List<Episode>> ConvertSeriesLinkToEpisodes(IndexResponse? indexResponse, string seriesUrl,
        string locale)
    {
    ...
    private async Task<List<Episode>> GetEpisodesFromSeasons(IndexResponse? indexResponse,
        IEnumerable<SeasonsResponse.SeasonItem> seasons, string locale)
    {
        ...
        foreach (SeasonsResponse.SeasonItem season in seasons)
        {
            ...
            Console.WriteLine(
                $"get season {season.SeasonNumber?.ToString() ?? "unknown"}  of {season.Title}  with {episodesResponse?.Episodes?.Count} episodes");
        }
