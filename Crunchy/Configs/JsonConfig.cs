...
public class JsonConfig : IConfigData
{
        ...
        LogDestination =
            Path.Join(
                string.IsNullOrEmpty(json?["log_destination"]?.ToString())
                    ? "."
                    : GetJsonValue(ref json, "log_destination"), "Crunchy.log");
    }
    ...
    private static string? GetJsonValue(ref JsonNode? json, string param)
    {
        return json?[param]?.GetValue<string>();
    }

    public static JsonObject PreRequirementJson()
    {
        return new()
        {
            ...
            ["show_log"] = true,
            ["log_destination"] = Path.Join(".", "Crunchy.log")
        };
    }
}